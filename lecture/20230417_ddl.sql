...
insert into emp01 (empno, enmae, sal) values(1111,'SON',1000);

select* from emp01;

-- 서브쿼리를 이용해서 기존 테이블의 구조로 테이블을 생성, 데이터도 저장, 
-- 제약조건은 복사되지 않는다!!!
create table emp02
as
select * from emp;
...
-- 컬럼 변경 (테이블에 컬럼을 추가, 테이블에 있는 컬럼을 삭제, 컬럼의 내용이나 이름을 변경)
...
-- not null 제약저건 : 설정한 컬럼에 null 값 입력을 금지하는 제약조건
-- 컬럼수준에서만 정의가 가능하다
drop table emp02;
-- 사원테이블 생성 : 사원번호, 사원이름, 직급, 부서번호
...
ename varchar2(10) constraint EMP01_ENAME_NN not null,
job varchar2(9) constraint EMP01_JOB_UK unique,
sal number(7,2) constraint EMP01_SAL_CK check(sal between 800 and 5000),
