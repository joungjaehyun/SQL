...
-- 오류 : not null 제약이 있는 컬럼은 반드시 데이터를 입력해 주어야 한다.
insert into dept01 (deptno,loc) values (30,'제주');

select * from dept01;

-- 모든 컬럼에 데이터를 입력하는 경우 -> 컬럼명을 생략학고 사용 할 수 있다.
insert into dept01 values (40,'기획팀','대전');

-- null 값의 입려
-- 명시적 입력
insert into dept01 values (50,'TEST',null);
insert into dept01 values (70,'TEST',null);
-- 암묵적
...
-- update 테이블이름 set 컬럼1=새로운값,컬럼2=새로운값 , .... where 행을선택하는 조건

select * from dept01;
-- 10번부서의 이름을 '디자인'으로 변경하고', 위치 정보를 '판교'로 수정하자.
update dept01 
...
-- SCOTT 사원의
-- 입사일자는 오늘로, 급여를 50으로 커미션을 4000 으로 수정합시다.
update emp01
...
-- 부서번호가 30번인 부서와 동일하게 변경하도록
update dept02
set (dname,loc) = (select dname, loc
                    from dept02 where deptno=30)
                    where deptno=20;
                    
                    
-- delete : 테이블의 행 단위 삭제
-- delete from 테이블이름 where 행을선택하는 조건
select * from dept02;
...
delete from emp01
where deptno =(select deptno from dept02
                where dname='SALES');
commit;
