...
-- 컬럼=값1 or 컬럽=값2 or 컬럼=값3 ...
select * from emp where deptno in(10, 20, 30);
select * from emp where deptno not in(10, 20);
-- 패턴 검색 : 문자열의 패턴 
-- 컬럼 LIKE '패턴'
-- 패턴 형식 : % -> 0개이상~, '%java%' => java 문자열을 포함하는 문자열
-- _ 한자리 __두자리 ___세자리 => '__A' 앞 두자리는 어떤 문자가 와도 상관없고 A로 끝나는 세자리 문자열

-- F로 시작하는 이름의 사원 리스트
select * from emp where ename like 'S%';
-- 이름에 A를 포함하는 사원 리스트 : '%A%'
select * from emp where ename like '%A%';
-- 이름에 A를 포함하지 않는 사원 리스트 : not like '%A%'
select * from emp where ename not like '%A%';
-- 이름에 N 으로 끝나는 사원 리스트 : '%N'
select * from emp where ename like '%N';

-- 사원 이름중 두번째 문자가 A 인 사원 리스트 : '_A%'
Select * from emp where ename like '_A%';
select * from emp where ename like '__R%';
select * from emp where ename like '%R_';

-- NULL 여부를 판단 하는 is null / is not null
-- 커미션이 등록되지 않은 사원 리스트 => comm이 null 인 사원
